...
let spotAssignments = document.getElementById('StudentAssignments');
let spotCourse = document.getElementById('StudentCourse');
...
function Spot(student,assignments, course) {
    this.student = student;
    this.assignments= assignments;
    this.course = course;
}
function spotToString(spot) {
        return (`${spot.student} ${spot.assignments} ${spot.course}`);
}
function submit(event){
        event.preventDefault();
        let newSpot=new Spot(spotStudent.value,spotAssignments.value,spotCourse.value);
        spots.push(newSpot);
...
function edit(event) {
        ...
        spotAssignments.value = spots[this.spotIndex].assignments;
...
function update(event) {
        event.preventDefault();
        spots[this.spotIndex] = new Spot(spotStudent.value,spotAssignments.value ,spotCourse.value);
        divList.innerHTML = '';
